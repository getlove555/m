...
#ki2 = ASUL.LINE()
#ki2.login(qr=True)
#ki2.login(token='EpMjQhJ8wSXq9BOECAH2.ZDFEjZ3fY8/74VuPEJeZmG.xvTfVI/xOMYnS5OHbwsdVoYLbZ+ijEexNNZ8jPl37is')
#ki2.loginResult()

#ki3 = ASUL.LINE()
#ki3.login(qr=True)
#ki3.login(token='Epe49r011YYgdDvIUFTd.X1JP/u4+7ITIHBKGuvDQJq.YZtnieM4R98icUJLHWWfp8gHhSnCHyhhaHxU+5+XCyU')
#ki3.loginResult()

#ki4 = ASUL.LINE()
#ki4.login(qr=True)
#ki4.login(token='EpE14HTk1VA9Xkp6BXL9.JfuEbCy+ichNj/SH4GPz6q.7GJMCegLNZiW6S6BOpn3ayY35LyVO0aGaNMKXp3twzI')
#ki4.loginResult()

#ki5 = ASUL.LINE()
#ki5.login(qr=True)
#ki5.login(token='EpE14HTk1VA9Xkp6BXL9.JfuEbCy+ichNj/SH4GPz6q.7GJMCegLNZiW6S6BOpn3ayY35LyVO0aGaNMKXp3twzI')
#ki5.loginResult()
...
#cl.loginResult()AgeCheckDocomoResult
...
Bots = [mid,kimid]#,ki3mid,ki4mid,ki5mid]
...
#----------------------------------------------------------
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["detectMention"] == True:
                     ...
                     balas = ["อย่าแท้กบ่อย",cName + " Ngapain Ngetag?",cName + " Nggak Usah Tag-Tag! Kalo Penting Langsung Pc Aja","-_-","Alin lagi off", cName + " Kenapa Tag saya?","SPAM PC aja " + cName, "Jangan Suka Tag gua " + cName, "Kamu siapa " + cName + "?", "Ada Perlu apa " + cName + "?","Tenggelamkan tuh yang suka tag pake BOT","Tersummon -_-"]
...
#-----------------------------------------------
              ...
              if msg.from_ in admin:
                ...
                cl.sendText(msg.to,"ʀᴇsᴘᴏɴ ᴏɴ ᴛᴀɢ ᴀʟʀᴇᴀᴅʏ")
            elif msg.text in ["Tag off","Autorespon:off","Respon off","Respon:off"]:
                wait["detectMention"] = False
                cl.sendText(msg.to,"ʀᴇsᴘᴏɴ ᴏғғ ᴛᴀɢ ᴀʟʀᴇᴀᴅʏ")      
...
#-----------------------------------------------------------
            elif msg.text in ["Rejectall","ลบรันทั้งหมด"]:
...
#-----------------------------------------------
                        ...
                        ki5.updateGroup(G)
#-----------------------------------------------
            elif msg.text in ["bye","Bye","กุเกลียดมึง","Sayonara"]:
                if msg.toType == 2:
                    ginfo = cl.getGroup(msg.to)
                    try:
                        ki.sendText(msg.to,"ไปก็ได้ บ๊ายบาย "  +  str(ginfo.name)  + "")
